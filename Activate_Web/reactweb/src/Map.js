...
class MapAPI extends Component {
  render() {
    ...
    const displayy = apiData && apiData.map((a) => parseInt(a['danger']) > 0 && console.log("위험!"));
    if(apiData && apiData.map((a) => parseInt(a['danger']) > 0)){
      console.log("위험");
    }
    const display = apiData && apiData.map((item) => <Marker key={item["timestamp"]} width='60px'icon={{ url: require("./장희선.png").default,
     scaledSize: new window.google.maps.Size(30, 30) }} position={{lat : parseFloat(item['latitude']), lng: parseFloat(item['longitude'])}}/>);
