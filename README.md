...
## 이용 방법
### 웹
- 링크 : http://happyvillage.s3-website.ap-northeast-2.amazonaws.com/
### 앱 
- 현재 구글 플레이에 출시 등록을 해둔 상태이며 검토중인 단계이다.
- 행복한동네.apk파일을 다운 받으면 이용 가능하다.
## 주요 기능
...
사용자가 입력했던 비밀번호와 비밀번호 확인란의 내용이 같으면  
PutUserData 함수가 호출되어 회원 가입란의 공란을 검사하지만,  
내용이 다를 시에는 “비밀번호를 다시 확인해주세요“ 라는 문구가 출력된다.  
...
## 향후 일정
어플의 환경 건의 카테고리에서 카메라 아이콘을 클릭하여 카메라를 활성화시키고  
사진과 함께 게시글을 업로드할 수 있는 기능이 추가될 것이다.  
또한 웹의 Environment 카테고리 역시 어플과 연동되어,  
유저가 어플을 통해 제보한 게시글을 웹에서도 확인할 수 있게끔 할 예정이다.  
